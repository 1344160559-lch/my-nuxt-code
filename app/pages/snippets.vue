<template>
  <div class="snippets-page">
    ...
    <div v-for="s in snippets" :key="s.id" class="snippet-item">
      ...
      <pre class="snippet-content">{{ s.content }}</pre>
    </div>
    <div v-if="showAdd" class="add-modal">
      <h3>新建代码片段</h3>
      <input v-model="addForm.title" placeholder="标题" />
      <input v-model="addForm.language" placeholder="语言" />
      <textarea v-model="addForm.content" placeholder="代码内容"></textarea>
      <textarea v-model="addForm.description" placeholder="描述"></textarea>
      <button @click="addSnippet">保存</button>
